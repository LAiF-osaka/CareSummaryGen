...
OUTPUT_DIR = "D:/work/LAiF/2_output/gs_hanwa/bakeneko_custom"
...
def send_request(patient_id: str, context: str):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            # タイムアウトを30分に延長
            response = requests.post(API_URL, json=payload, timeout=18000)
...
def process_patient_files():
    ...
    for i, patient_id in enumerate(patient_ids):
        ...
        if os.path.exists(output_file_path):
            print(
                f"出力ファイルが既に存在するため、患者ID: {patient_id} をスキップします。"
            )
            continue
        ...
        if summary:
            try:
                ...
                with open(output_file_path, "w", encoding="utf-8") as f:
                    f.write(summary)
                print(f"サマリを {output_file_path} に保存しました。")
            except Exception as e:
